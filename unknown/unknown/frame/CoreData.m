...
//  chaping
//
//  Created by spzhong on 14/12/30.
...
+(BOOL)delete_Data:(NSString*)model withWhere_id:(NSString*)where
{
    ...
    for (int i = 0; i < mutableFetchResults.count; i++) {
        [[CoreData getDele].managedObjectContext deleteObject:[mutableFetchResults objectAtIndex:i]];
    }
...
+(NSManagedObject *)selcet_OneData:(NSString *)model withWhere_id:(NSString *)where{
    NSMutableArray *allArray = [CoreData selcet_ALLData:model withNSPredicate:[CoreData creat_NSPredicate:where] withNSSortDescriptor:nil withPage:-1];
    if ([allArray count]==0) {
        return nil;
    }
